...
Known critical risks: overlapping architectural layers and dead-looking duplicate frontend; default-on LLM planning with broad exception conversion; extensive LLM plan mutation; in-memory request history breaks after restart; output persistence is non-transactional; unprofiled uploads; role binding is heuristic; frontend ignores several backend options; deployment exposes no host ports in Compose despite README URLs. The full backend suite currently runs in a normal local/CI process (1,627 passed, one unrelated dependency-contract assertion failed); in-process FastAPI `TestClient` requests can stall in the restricted Codex sandbox, so use a normal process and bounded test commands. See `docs/test-harness-diagnosis.md`.
